<?php
/**
 * Corso BG5 Foundation — Raccolta dati post-pagamento
 * Query params: ?session_id=cs_xxx&course=bg5-foundation
 *
 * BOZZA — la sezione "recesso/condizioni" qui sotto è un placeholder generico,
 * NON il testo legale definitivo. Corso strutturato (20 lezioni su 2 semestri):
 * la base giuridica per il recesso richiede revisione legal-guardian prima di
 * andare live (diversa sia dal contenuto digitale istantaneo del Libretto sia
 * dalla singola sessione delle Letture).
 */
...
<?php else: ?>
  ...
  <div class="ld-confirm-box">
    ...
    <div class="ld-confirm-text">
      <strong>Ti stai iscrivendo a: <?= htmlspecialchars($product['name'], ENT_QUOTES) ?></strong><br>
      Pagamento confermato (<?= $priceLabel ?>). Ora servono i tuoi dati per completare l'iscrizione.
    </div>
  </div>
  ...
  <form class="ld-form" method="POST" action="/corso-dati/invia.php" id="ld-form">
    ...
    <p class="ld-section-title">Dati per la fattura (facoltativo)</p>
    <div class="ld-group">
      <label class="ld-label" for="ld-cf">Codice fiscale o P.IVA</label>
      <input class="ld-input" type="text" id="ld-cf" name="codice_fiscale" autocomplete="off" placeholder="es. RSSMRA80A01H501U">
    </div>
    <div class="ld-group">
      <label class="ld-label" for="ld-indirizzo">Indirizzo di residenza</label>
      <input class="ld-input" type="text" id="ld-indirizzo" name="indirizzo" autocomplete="street-address" placeholder="Via, città, CAP">
    </div>

    <div class="ld-group">
      <label class="ld-label" for="ld-msg">Note per Valentina <span class="ld-hint" style="font-style:italic">(opzionale)</span></label>
      <textarea class="ld-textarea" id="ld-msg" name="messaggio" rows="5" placeholder="Qualcosa che vuoi farle sapere prima del corso..."></textarea>
    </div>
