---
title: Plesso Solare definito o indefinito
template: glossario
published: true
routable: true
ordine: 3
seo_title: 'Plesso Solare Human Design: l''onda emotiva e cosa cambia se è bianco'
seo_desc: 'Il Plesso Solare nello Human Design: l''onda emotiva di chi lo ha definito, l''amplificazione delle emozioni altrui di chi lo ha aperto, l''Autorità Emotiva.'
description: 'Il centro delle emozioni. Chi lo ha definito non ha mai la verità adesso: gli arriva col tempo, attraversando l''onda.'
richiamo: Se decidi spesso a caldo e poi te ne penti, il <a href="/libretto-istruzioni">Libretto d&rsquo;Istruzioni</a> ti dice se hai l&rsquo;Autorit&agrave; Emotiva e quanto tempo ti serve davvero prima di rispondere.
didascalia: Il Plesso Solare e le sue sette porte
alt_immagine: 'Dettaglio del bodygraph: il Plesso Solare con le porte 6, 22, 30, 36, 37, 49 e 55'
chiusa_titolo: Decidi nell'onda o nell'istante?
chiusa_testo: Nella Prima Lettura vediamo la tua Autorità Decisionale e come si applica alle scelte che stai rimandando.
faq:
- question: Cosa rappresenta il Plesso Solare nello Human Design?
  answer: 'È il centro delle emozioni ed è insieme un motore. Non è il plesso solare dell''anatomia né il chakra omonimo: nello Human Design indica il modo in cui una persona vive l''onda emotiva e, se il centro è definito, come prende le decisioni.'
- question: Cos'è l'onda emotiva?
  answer: 'È il movimento naturale delle emozioni di chi ha il Plesso Solare definito: sale e scende nel tempo, con propri alti e bassi. Per queste persone la chiarezza non esiste nel momento, arriva attraversando l''onda.'
- question: Quante persone hanno il Plesso Solare definito?
  answer: Circa il 47%. Chi lo ha definito ha sempre l'Autorità Emotiva, che ha la precedenza su tutte le altre.
- question: Chi ha il Plesso Solare aperto è meno emotivo?
  answer: Spesso è il contrario. Non produce l'onda ma assorbe e amplifica quella degli altri, quindi può sentire le emozioni di una stanza con più intensità di chi le sta vivendo.
nome_breve: Plesso Solare
apertura: |
  Il **Plesso Solare** è il triangolo a destra nel bodygraph, ed è l'unico centro che fa due mestieri: è un motore, quindi produce energia, ed è un centro di consapevolezza, quindi elabora informazione. L'informazione che elabora sono le emozioni.

  Circa il 47% delle persone ce l'ha definito.
scheda:
  definito: 47
  aperto: 53
  tipo: Motore e consapevolezza
  nota_tipo: l'unico che fa entrambe le cose
  porte: 6, 22, 30, 36, 37, 49, 55
  n_porte: 7
  autorita: Emotiva
  nota_autorita: ha la precedenza su tutte
definito:
  titolo: La chiarezza arriva col tempo
  testo: |
    Hai un'onda tua. Le emozioni salgono e scendono seguendo un ritmo che non dipende da quello che succede intorno: puoi svegliarti giù in una giornata perfetta e su in una giornata storta. Non è instabilità, è meccanica.

    La conseguenza pratica è la più importante di tutta la carta: **per te la verità del momento non esiste**. Quello che senti adesso è un punto dell'onda, non la conclusione. Chi ha questo centro definito ha l'**Autorità Emotiva**, che nel **BG5 (Business Group 5)** rientra in quella che si chiama Autorità Decisionale, e la sua regola è una sola: aspettare. Non c'è una formula sul quanto, ma c'è un segnale, ed è la calma che arriva quando l'onda ha finito il giro.

    La sfida quotidiana è resistere alla pressione di rispondere subito. Ogni "mi serve una risposta adesso" è, per te, una richiesta di decidere nel punto sbagliato dell'onda.
aperto:
  titolo: Senti le emozioni della stanza
  testo: |
    Non produci l'onda: la ricevi. Entri in una stanza e la temperatura emotiva ti arriva addosso amplificata, spesso prima che qualcuno dica una parola. Per questo chi ha il Plesso Solare aperto viene considerato empatico, e in parte lo è: sente davvero.

    Il condizionamento è di due tipi e vanno insieme. Il primo è **prendere per proprie emozioni che sono di altri**, e portarsele a casa. Il secondo è **evitare il conflitto per non alterare l'atmosfera**: dire di sì per non guastare la serata, rimandare la conversazione difficile, tenere la pace anche a costo di sparire.

    La saggezza che arriva con gli anni è saper distinguere le due cose e chiedersi, ogni volta: **questa emozione è mia o l'ho presa entrando?**
---

## Nel lavoro

Nel **BG5** questo centro cambia il modo di condurre le trattative e le decisioni di gruppo. Una persona con Autorità Emotiva messa a decidere in riunione, seduta stante, deciderà male: non perché sia lenta, ma perché il suo meccanismo richiede il tempo dell'onda. La stessa domanda il giorno dopo produce una risposta diversa e più solida.

Chi ha il centro aperto tende invece a sentire il clima del gruppo prima degli altri, e questa è un'informazione preziosa a patto di ricordare che quel clima non è suo.

## Come si legge nella tua carta

Il Plesso Solare è il triangolo in basso a destra, collegato dalle porte 6, 22, 30, 36, 37, 49 e 55.

Il [calcolo gratuito](/calcolo-human-design) ti dice se è colorato e quale Autorità ne deriva.
