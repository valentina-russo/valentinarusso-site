---
title: 'Autorità Mentale: decidere attraverso l''ambiente'
template: glossario
published: true
routable: true
ordine: 6
nome_breve: Mentale
seo_title: 'Autorità Mentale: come decide un Proiettore (Guida)'
seo_desc: 'L''Autorità Mentale nello Human Design: i Proiettori (Guide) senza centri motori definiti decidono attraverso le conversazioni e l''ambiente, non dall''interno.'
description: Nessuna autorità interna. La chiarezza arriva parlando con le persone giuste, nei posti giusti.
richiamo: Se decidi bene solo dopo averne parlato con qualcuno, il <a href="/libretto-istruzioni">Libretto d&rsquo;Istruzioni</a> ti dice con chi e in che ambienti il tuo disegno funziona meglio.
didascalia: 'L''Autorità Mentale: nessun centro decisionale definito'
alt_immagine: Bodygraph con Testa e Ajna colorate e tutti i centri sotto la Gola aperti
chiusa_titolo: La tua carta non ha un'autorità interna?
chiusa_testo: Nella Prima Lettura vediamo con chi e dove le tue decisioni prendono forma, invece di aspettare una certezza che non arriva.
faq:
- question: Cos'è l'Autorità Mentale nello Human Design?
  answer: 'È la condizione di alcuni Proiettori (Guide nel BG5) che non hanno definito nessuno dei centri che generano un''Autorità. Non significa decidere con la testa: la chiarezza arriva attraverso più conversazioni e l''ambiente giusto.'
- question: Se non ho un'autorità interna, come decido?
  answer: Parlando della questione con persone diverse, in momenti e luoghi diversi, e osservando quale posizione resta stabile. Il luogo conta quanto l'interlocutore.
- question: L'Autorità Mentale vale anche per i Riflettori (Valutatori)?
  answer: 'No. I Riflettori (Valutatori nel BG5) non hanno nessuna Autorità: decidono aspettando un ciclo lunare intero.'
- question: Come si interpreta lo Human Design?
  answer: Si parte dalla Tipologia (Tipo di Carriera nel BG5), dalla Strategia e dall'Autorità. I Centri spiegano il dettaglio, ma le decisioni si prendono con l'Autorità.
apertura: |
  L'**Autorità Mentale**, chiamata anche Ambientale, è quella dei Proiettori (Guide) che non hanno definito nessuno dei centri che generano un'Autorità. Nella carta è la sesta possibilità dell'ordine di precedenza, ed è rara.

  Il nome inganna: **non significa decidere con la testa**. Significa che non c'è un punto interno a cui affidarsi, e che la chiarezza si costruisce fuori.
scheda:
- etichetta: Diffusione
  valore: Rara
  nota: solo alcuni Proiettori (Guide)
- etichetta: Da quale centro
  valore: Nessuno
  nota: nessun centro sotto la Gola definito
- etichetta: Chi la ha
  valore: Proiettori (Guide)
  nota: (Guide nel BG5)
- etichetta: Quanto aspettare
  valore: Più conversazioni
  nota: e il posto giusto
definito:
  etichetta: Come funziona
  titolo: La chiarezza si costruisce parlando
  testo: |
    Chi ha questa Autorità arriva alla decisione **attraverso più conversazioni**, con persone diverse e in momenti diversi. Non per raccogliere consigli: per sentire come cambia la propria posizione mentre la racconta.

    Conta moltissimo **dove** avvengono quelle conversazioni. Con tutti i centri aperti, queste persone assorbono l'ambiente in modo totale: lo stesso discorso in una stanza sbagliata dà una conclusione sbagliata. Il luogo, in questa Autorità, non è un dettaglio.

    Il tempo qui è un alleato: si decide dopo aver girato la questione da più lati e in più posti, mai al primo giro.
aperto:
  etichetta: Dove si sbaglia
  titolo: L'errore che costa di più
  testo: |
    **Decidere da soli, in fretta, in un posto qualunque.** È il modo più diretto per prendere una decisione che non è tua ma dell'ultima persona con cui hai parlato.

    L'altro errore è cercare la certezza. Con la mente aperta non arriva mai, e aspettarla significa non decidere mai. Quello che arriva, invece, è una posizione che resta stabile mentre la racconti a persone diverse: quella è la chiarezza disponibile, ed è sufficiente.
nota_valentina: '<b>Anche la mia carta dice questo.</b> Sono un Proiettore (Guida nel BG5) con Autorit&agrave; Mentale: nessuna autorit&agrave; interna, e le decisioni che prendono forma parlando. Lo conosco da dentro, non solo dai libri. Se ti ci sei riconosciuto, nella Prima Lettura partiamo da qui.'
---

## Nel lavoro

Nel **BG5 (Business Group 5)** queste sono le persone che vedono il sistema meglio di chi ci sta dentro, proprio perché non hanno definizione che li fissi. Sono osservatori naturali, e il loro contributo migliore è la lettura d'insieme.

Chiedere loro una decisione secca in riunione è il modo peggiore di usarli. Chiedere la stessa cosa dopo che ne hanno parlato con tre persone in due giorni dà una risposta di tutt'altra qualità.

## Come si legge nella tua carta

Serve che nessuno fra [Plesso Solare](/human-design/centri/plesso-solare), [Sacrale](/human-design/centri/sacrale), [Milza](/human-design/centri/milza), [Cuore](/human-design/centri/cuore) e [Centro G](/human-design/centri/g) collegato alla Gola sia definito.

Il [calcolo gratuito](/calcolo-human-design) te lo mostra subito.
