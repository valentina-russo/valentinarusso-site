---
title: Le 7 Autorità dello Human Design
template: glossario
published: true
routable: true
ordine: 2
nome_breve: Autorità
seo_title: 'Le 7 Autorità dello Human Design: qual è la tua'
seo_desc: 'Emotiva, Sacrale, Splenica, dell''Ego, Autoproiettata, Mentale, e il caso di chi non ne ha nessuna: le Autorità Decisionali dello Human Design, una per una.'
description: 'L''Autorità è il modo in cui il tuo corpo decide. Non è la mente: è il punto della carta a cui spetta l''ultima parola.'
didascalia: I tre centri da cui nascono le Autorità più diffuse
alt_immagine: Bodygraph con Plesso Solare, Sacrale e Milza colorati, gli altri centri aperti
etichetta_figlie: Le sette
titolo_figlie: Trova la tua
richiamo: 'Il <a href="/libretto-istruzioni">Libretto d&rsquo;Istruzioni</a> parte proprio da qui: come decidere secondo il tuo disegno, applicato alle scelte che hai davanti adesso.'
chiusa_titolo: Non sai qual è la tua?
chiusa_testo: Il calcolo gratuito te la dice in pochi secondi, insieme alla Tipologia (Tipo di Carriera nel BG5) e ai centri definiti.
faq:
- question: Quali sono le 7 Autorità dello Human Design?
  answer: 'Emotiva, Sacrale, Splenica, dell''Ego, Autoproiettata, Mentale. La settima voce non è un''Autorità ma la sua assenza: i Riflettori (Valutatori nel BG5) non ne hanno nessuna e decidono con il ciclo lunare.'
- question: Che cos'è l'Autorità nello Human Design?
  answer: 'È il meccanismo con cui il corpo prende una decisione, e nel BG5 (Business Group 5) si chiama Autorità Decisionale. Non è un''opinione né un ragionamento: è il punto della carta a cui spetta l''ultima parola, e la mente non è mai quel punto.'
- question: Come si interpreta lo Human Design?
  answer: 'Si parte da tre cose: la Tipologia (Tipo di Carriera nel BG5), la Strategia che ne deriva e l''Autorità. I nove Centri e le 64 porte servono a spiegare il dettaglio, ma le decisioni si prendono con l''Autorità.'
- question: Quali sono i 9 Centri dello Human Design?
  answer: Testa, Ajna, Gola, Centro G, Cuore, Milza, Sacrale, Plesso Solare e Radice. Quattro di questi, più il Centro G, possono generare un'Autorità.
---

L'**Autorità** è il modo in cui il tuo corpo decide. Nello Human Design non è una qualità morale né una posizione di comando: è un meccanismo, e sta scritto nella carta. Nel **BG5 (Business Group 5)** si chiama **Autorità Decisionale**, ed è la stessa cosa applicata al lavoro.

Le Autorità sono sette, e ognuno ne ha una sola. Quale sia dipende da quali centri hai colorati, in un ordine di precedenza fisso: se il Plesso Solare è definito hai l'Autorità Emotiva e nessun'altra, comunque sia fatto il resto della carta. Se è aperto si guarda il Sacrale, poi la Milza, poi il Cuore, e così via.

## L'ordine di precedenza

Si scorre dall'alto e ci si ferma al primo caso che corrisponde alla tua carta.

1. **Plesso Solare** definito → Autorità Emotiva
2. **Sacrale** definito → Autorità Sacrale
3. **Milza** definita → Autorità Splenica
4. **Cuore** definito → Autorità dell'Ego
5. **Centro G** collegato alla Gola → Autorità Autoproiettata
6. nessuno dei precedenti, per certi Proiettori (Guide) → Autorità Mentale
7. tutti i nove centri aperti, i Riflettori (Valutatori) → nessuna Autorità, si decide col ciclo lunare

Non è una classifica di valore: è la sequenza con cui si legge la carta. Le prime tre coprono la grande maggioranza delle persone, le ultime quattro sono rare.

## Perché la mente non è mai l'Autorità

È il punto che spiazza di più chi si avvicina allo Human Design. La mente serve a elaborare, comunicare e influenzare, e in questo è formidabile. Non è però attrezzata per sapere cosa è giusto per te: raccoglie informazioni dall'esterno, e quello che produce sono argomenti, non risposte.

Nel lavoro questo si vede bene. Una decisione presa perché "ha senso sulla carta" e contro quello che il corpo diceva è la storia di buona parte dei cambi di lavoro finiti male, e nel **BG5** è il primo posto dove si va a guardare quando un ruolo non funziona.

## Come si legge nella tua carta

Il [calcolo gratuito](/calcolo-human-design) te la dice subito, insieme alla Tipologia e ai centri definiti.
